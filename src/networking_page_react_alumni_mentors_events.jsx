  ...
  return (
    <button onClick={onSelect} className={`w-full text-left rounded-xl border p-3 hover:bg-slate-50 transition ${selected ? "bg-slate-50 border-slate-300" : "bg-white"}`}>
      ...
      <div className="mt-2 text-xs text-muted-foreground flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1"><GraduationCap className="h-3.5 w-3.5"/> {p.degree}</span>
        <span className="inline-flex items-center gap-1"><MapPin className="h-3.5 w-3.5"/> {p.location}</span>
      </div>
      <div className="mt-2 flex flex-wrap gap-1.5">
        {p.skills.slice(0,3).map((s)=> <Badge key={s} variant="outline" className="rounded-full">{s}</Badge>)}
      </div>
    </button>
  );
}
      ...
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{p.headline} · {p.degree} {p.classYear}</p>
            <p className="text-sm text-muted-foreground"><MapPin className="inline h-4 w-4 mr-1"/>{p.location}</p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" className="gap-2"><MessageSquare className="h-4 w-4"/> Message</Button>
            <Button className="gap-2">Request Chat <PhoneCall className="h-4 w-4"/></Button>
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          {p.skills.map((s)=> <Badge key={s} className="rounded-full" variant="outline">{s}</Badge>)}
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <section>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {p.willingTo.map((w)=> <Badge key={w} variant="secondary" className="rounded-full">{w}</Badge>)}
          </div>
        </section>
        <section>
          <h3 className="font-medium">Links</h3>
          <div className="mt-2 flex flex-wrap gap-3 text-sm">
            {p.links?.linkedin && <a href={p.links.linkedin} className="inline-flex items-center gap-1 underline"><Linkedin className="h-4 w-4"/> LinkedIn</a>}
            {p.links?.website && <a href={p.links.website} className="inline-flex items-center gap-1 underline"><Globe className="h-4 w-4"/> Website</a>}
            {p.links?.email && <a href={`mailto:${p.links.email}`} className="inline-flex items-center gap-1 underline"><Mail className="h-4 w-4"/> Email</a>}
          </div>
        </section>
        <section>
          <h3 className="font-medium">Suggested intro</h3>
          <p className="text-sm text-muted-foreground mt-1">“Hi {p.name.split(" ")[0]}, I’m a current student interested in {p.skills[0]}. Would you be open to a 15‑minute chat about your path into {p.headline.split("@")[0].trim()}? I can work around your schedule.”</p>
        </section>
      </CardContent>
      <CardFooter className="justify-between">
        <div className="text-xs text-muted-foreground">Be respectful. Alumni volunteer their time.</div>
        <Button variant="ghost" className="gap-2">Book via Calendly <ExternalLink className="h-4 w-4"/></Button>
      </CardFooter>
    </Card>
  );
}
    ...
    <Card className="rounded-2xl">
      ...
      <CardContent className="text-sm text-muted-foreground">
        <div className="flex flex-wrap items-center gap-3">
          <span className="inline-flex items-center gap-1"><Calendar className="h-4 w-4"/>{e.when}</span>
          <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4"/>{e.location}</span>
        </div>
        <div className="mt-2 flex flex-wrap gap-2">
          {e.tags.map((t)=> <Badge key={t} variant="outline" className="rounded-full">{t}</Badge>)}
        </div>
      </CardContent>
      <CardFooter>
        <Button className="gap-2 w-full">RSVP <ChevronRight className="h-4 w-4"/></Button>
      </CardFooter>
    </Card>
  );
}
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-4 grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <Card className="rounded-2xl h-[calc(100vh-160px)] overflow-hidden">
            <CardHeader className="py-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold">{filtered.length} connection{filtered.length!==1?"s":""}</h2>
                <a className="text-xs text-primary inline-flex items-center gap-1" href="#">Advanced filters <Filter className="h-3.5 w-3.5"/></a>
              </div>
            </CardHeader>
        ...
        <section className="lg:col-span-7 xl:col-span-8 space-y-4">
          {selected ? (
            ...
            <Card className="rounded-2xl"><CardContent className="py-12 text-center text-muted-foreground">Choose a person to view details.</CardContent></Card>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <a className="text-xs text-primary inline-flex items-center gap-1" href="#">See all <ChevronRight className="h-3.5 w-3.5"/></a>
            </div>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto max-w-7xl px-4 py-8 text-sm text-muted-foreground">
          Tip: Be specific in your outreach — mention a shared class, club, or interest.
        </div>
      </footer>
    </div>
  );
