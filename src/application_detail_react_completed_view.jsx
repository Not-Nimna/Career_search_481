...
const MOCK_APP = {
  ...
  answers: {
    ...
    project:
      "Built a ROS2-based perception pipeline: camera capture → preprocessing → YOLOv8 for object detection → EKF-based tracking. Wrote nodes in Python/C++, used Docker + CI for reproducible builds.",
  },
  timeline: [
      ...
      <header className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="gap-2" onClick={()=> alert("Exported as PDF (mock)") }>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 grid gap-6 lg:grid-cols-12">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground flex flex-wrap items-center gap-2 mt-1">
                    <span className="inline-flex items-center gap-1"><Building2 className="h-4 w-4"/> {app.job.company}</span>
                    <span>•</span>
                    <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4"/> {app.job.location}</span>
                  </p>
                </div>
                <div className="text-right min-w-[220px]">
                  <div className="text-xs text-muted-foreground">Submitted</div>
                  <div className="font-medium">{fmtDate(app.submittedISO)} · {fmtTime(app.submittedISO)}</div>
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="grid gap-3 text-sm">
              <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl border">
                <div className="inline-flex items-center gap-2"><FileText className="h-4 w-4"/> Resume</div>
                <div className="text-muted-foreground">{app.documents.resume.name} · {app.documents.resume.sizeMB.toFixed(2)} MB</div>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="secondary" className="gap-2"><FileDown className="h-4 w-4"/> Download</Button>
                  <Button size="sm" variant="ghost" className="gap-2"><ExternalLink className="h-4 w-4"/> Open</Button>
                </div>
              </div>
              <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl border">
                <div className="inline-flex items-center gap-2"><FileText className="h-4 w-4"/> Transcript</div>
                <div className="text-muted-foreground">{app.documents.transcript.name} · {app.documents.transcript.sizeMB.toFixed(2)} MB</div>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="secondary" className="gap-2"><FileDown className="h-4 w-4"/> Download</Button>
                  <Button size="sm" variant="ghost" className="gap-2"><ExternalLink className="h-4 w-4"/> Open</Button>
                </div>
              ...
              {app.documents.cover && (
                <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-xl border">
                  <div className="inline-flex items-center gap-2"><FileText className="h-4 w-4"/> Cover letter</div>
                  <div className="text-muted-foreground">{app.documents.cover.name} · {app.documents.cover.sizeMB.toFixed(2)} MB</div>
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="secondary" className="gap-2"><FileDown className="h-4 w-4"/> Download</Button>
                    <Button size="sm" variant="ghost" className="gap-2"><ExternalLink className="h-4 w-4"/> Open</Button>
                  </div>
                  ...
                  <div className="grid gap-2">
                    {app.documents.extras.map((f) => (
                      <div key={f.name} className="flex items-center justify-between gap-2 text-sm">
                        <span className="inline-flex items-center gap-2"><Paperclip className="h-4 w-4"/>{f.name}</span>
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={()=> { if (note.trim()) { alert("Saved (mock)"); setNote(""); } }}>Save</Button>
              </div>
            </CardContent>
          </Card>
        </section>
        ...
        <aside className="lg:col-span-4 space-y-4">
              ...
              {app.timeline.map((t) => (
                <div key={t.when} className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{fmtDate(t.when)} · {fmtTime(t.when)}</div>
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="text-sm text-muted-foreground space-y-2">
              <div className="flex items-center gap-2"><User2 className="h-4 w-4"/> Recruiting Team</div>
              <div className="flex items-center gap-2"><Mail className="h-4 w-4"/> careers@aurora‑robotics.example</div>
              <div className="flex items-center gap-2"><Link className="h-4 w-4"/> <a className="underline" href="/jobs/aurora-robotics-lab">Job posting</a></div>
            </CardContent>
          </Card>
        </aside>
      </main>
