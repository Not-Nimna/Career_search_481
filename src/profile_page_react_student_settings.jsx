    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent>
              <div
                ...
                onDrop={onDropAvatar}
                className="relative rounded-2xl border p-4 bg-white text-center"
              >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["Internship","Co-op","Full-time"].map((opt)=> (
                    <button key={opt} className={`px-2.5 py-1 rounded-full border text-sm ${profile.seeking.has(opt)?"bg-slate-900 text-white border-slate-900":"bg-white"}`} onClick={()=> setProfile({...profile, seeking: toggleSet(profile.seeking, opt)})}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["Remote","Hybrid","On-site"].map((opt)=> (
                    <button key={opt} className={`px-2.5 py-1 rounded-full border text-sm ${profile.workMode.has(opt)?"bg-slate-900 text-white border-slate-900":"bg-white"}`} onClick={()=> setProfile({...profile, workMode: toggleSet(profile.workMode, opt)})}>{opt}</button>
                  ))}
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" onClick={()=> { if(roleInput.trim()){ setProfile({...profile, roles: addTag(profile.roles, roleInput)}); setRoleInput(""); } }} className="gap-2"><Plus className="h-4 w-4"/> Add</Button>
                </div>
              </div>
              {/* Skills */}
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" onClick={()=> { if(skillInput.trim()){ setProfile({...profile, skills: addTag(profile.skills, skillInput)}); setSkillInput(""); } }} className="gap-2"><Plus className="h-4 w-4"/> Add</Button>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto max-w-6xl px-4 py-6 text-sm text-muted-foreground">
          Keep your details up‑to‑date — employers see your preferred roles and latest resume.
        </div>
      </footer>
    </div>
  );
